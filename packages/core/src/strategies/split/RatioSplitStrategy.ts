...
export class RatioSplitStrategy implements SplitStrategy {
  readonly key = 'ratio';
  ...
  compute(
    ...
    return performSplit(state, tile, tileId, orientation, clampedRatio);
  }
}
...
function performSplit(
  ...
  if (orientation === 'vertical') {
    ...
    a = new Tile({
      id: `${String(tileId)}-ratio-a` as TileId,
      x: tile.x,
      y: tile.y,
      width: w1,
      height: tile.height,
    ...
    b = new Tile({
      id: `${String(tileId)}-ratio-b` as TileId,
      x: tile.x + w1,
      y: tile.y,
      width: w2,
      height: tile.height,
